...
#
# KEY CHANGES vs original DFM_model.R:
#   1. Test window: 2019Q1-2024Q2 (was 2023Q1-2024Q2 — too short)
#   2. ROLLING=TRUE: model re-estimated each origin (was estimated once only)
#   3. FALLBACK: if DFM fails (Lapack error), uses previous model (no NA)
#   4. DFM_MAX_ITER=100: faster, sufficient convergence (was 500)
#   5. Lags [-2,-1,0] only: consistent with all other models
#   6. gen_lagged_data: date-based cutoff (was positional — bug with gaps)
#   7. ARMA comparison removed: done in comparison_final.py
#
...
# --- FIXED (thesis design - do not change) -----------------------------------
TARGET_VARIABLE <- "GDP"
LAGS            <- c(-2L, -1L, 0L)    # only genuine nowcast vintages
TRAIN_START     <- as.Date("2005-01-01")
TEST_START      <- as.Date("2019-03-01")
TEST_END        <- as.Date("2024-06-01")
CRISIS_START    <- as.Date("2019-01-01")
CRISIS_END      <- as.Date("2021-06-01")
NORMAL_START    <- as.Date("2022-01-01")
NORMAL_END      <- as.Date("2024-06-01")
...
# classify_period: label dates as crisis / normal / other.
...
# =============================================================================
# Temporal validity:
#   [ok] No leakage    - training ends one quarter before nowcast date
...
#   fails. Document this in the paper's methodology section.
...
n_fallback     <- 0L     # count of times fallback was used (for reporting)
...
for (i in seq_along(test_dates)) {
  ...
  actual_val <- data_model %>%
    dplyr::filter(format(date, "%Y-%m") == format(nowcast_date, "%Y-%m")) %>%
  ...
  # ---- Training window -------------------------------------------------------
  # Ends one full quarter before the nowcast date.
  train_cutoff <- seq(nowcast_date, by = "-3 months", length = 2)[2]
  ...
  # FALLBACK LOGIC:
  #   If estimation succeeds  → update output_dfm with new model
  #   If estimation fails     → keep previous output_dfm (fallback)
  #   This prevents NA predictions when Lapack singular matrix errors occur
  #   (common when COVID shock enters the training set).
  ...
  if (should_estimate) {
    ...
    if (!is.null(output_dfm_new)) {
      # Estimation succeeded — update model
  ...
  for (lag in LAGS) {
    tryCatch({
      ...
      vintage_data[
        format(vintage_data$date, "%Y-%m") == format(nowcast_date, "%Y-%m"),
        TARGET_VARIABLE
      ] <- NA
      ...
      # Extract prediction for the nowcast quarter
      prediction <- dfm_output %>%
        dplyr::filter(
          format(date, "%Y-%m") == format(nowcast_date, "%Y-%m")
        ) %>%
        dplyr::pull(!!TARGET_VARIABLE)
...
for (period in c("crisis", "normal")) {
  label <- if (period == "crisis") "Crisis (2019Q1-2021Q2)"
  else "Normal (2022Q1-2024Q2)"
...
# ---- Print results ----------------------------------------------------------
cat("\n", strrep("=", 60), "\n", sep = "")
cat("FULL TEST PERIOD - DFM (2019Q1-2024Q2)\n")
cat(strrep("=", 60), "\n", sep = "")
...
p_crisis <- ggplot() +
  ...
  labs(title    = "DFM - Crisis Period (2019Q1-2021Q2)",
...
p_normal <- ggplot() +
  ...
  scale_linetype_manual(
    ...
    labels = LAG_LABELS, name = "Vintage") +
  scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
  labs(title    = "DFM - Non-Crisis Period (2022Q1-2024Q2)",
       subtitle = "Nowcasting Vintages | El Salvador | full series for context",
       x = "Date", y = "GDP growth rate (QoQ)") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "bottom",
...
p_heatmap <- ggplot(heatmap_data,
  ...
  geom_text(aes(label = round(RMSE, 5)), size = 3.5) +
...
# =============================================================================
# Purpose : DFM Factor Loadings — Variable Importance Analysis
#
# What this block does:
#   1. Extracts factor loadings from the output_dfm object (already in memory)
#   2. Saves the model to disk for future use (saveRDS)
...
#
# IMPORTANT: This block assumes that output_dfm and vars_in_model
#             are in memory (R was not closed since dfm_final.R was run)
#
...
# =============================================================================
# STEP 1: Save the DFM object to disk (to avoid losing it if R is closed)
# =============================================================================

saveRDS(output_dfm,
        file = file.path(OUT_DIR, "dfm_model_object.rds"))
cat("DFM model object saved to:",
    file.path(OUT_DIR, "dfm_model_object.rds"), "\n")
cat("(To reload in a new session: output_dfm <- readRDS('...dfm_model_object.rds'))\n\n")
...
# =============================================================================
# =============================================================================
# DFM Factor Loadings — corrected for nowcastDFM structure
# C matrix has 223 columns = 4 blocks x multiple lags
# We only need the FIRST column per block (contemporaneous loading)
# =============================================================================
...
# nowcastDFM expands C into [n_vars x (n_blocks * p_lags)]
# The blocks object tells us which columns belong to which factor
# We extract only the FIRST column per block = contemporaneous loading

n_blocks <- ncol(block_cols)   # = 4 (block_g, block_s, block_r, block_l)
cat("Number of blocks:", n_blocks, "\n")
cat("C matrix dimensions:", nrow(output_dfm$C), "x", ncol(output_dfm$C), "\n")

# The C matrix is organized as:
# cols 1 to p belong to block 1, cols p+1 to 2p belong to block 2, etc.
# p = number of lags in state equation
p_lags   <- ncol(output_dfm$C) / n_blocks
cat("Lags per block (p):", p_lags, "\n")

# Extract contemporaneous loadings: first column of each block
block_names  <- c("Factor G (Global/Real)",
                  "Factor S (External Sector)",
                  "Factor R (Financial/Monetary)",
                  "Factor L (Labour/Fiscal)")

# Index of first column for each block
block_starts <- seq(1, ncol(output_dfm$C), by = p_lags)
cat("Block start indices:", block_starts, "\n")

# Build clean loadings dataframe with one column per block
...
# Add mean absolute loading across all blocks
loadings_df <- loadings_df %>%
  dplyr::mutate(
  ...
  p <- ggplot(top_vars,
    ...
    labs(
      ...
      subtitle = paste0(
        ...
        "Last training window (ending 2024Q1)\n",
